...
class MyWechatPay {
    ...
    public static function unifiedOrder ($inputObj, $timeOut = 6) {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url(C('WECHAT_INFO.notify_url'));
        }

        // 公众账号ID
        $inputObj->SetAppid(C('WECHAT_INFO.appid'));
        // 商户号
        $inputObj->SetMch_id(C('WECHAT_INFO.mchid'));
        // 终端ip
    ...
    public static function withdraw ($inputObj, $timeOut = 6) {
        ...
        // 检测关联参数
        if ($inputObj->GetCheck_name() === 'FORCE_CHECK' || $inputObj->GetCheck_name() === 'OPTION_CHECK') {

            if (!$inputObj->IsRe_user_nameSet()) {
                throw new Exception("缺少企业付款接口必填参数re_user_name！");
            }
        ...
        $inputObj->SetAppid(C('WECHAT_INFO.appid'));
        // 商户号
        $inputObj->SetMchid(C('WECHAT_INFO.mchid'));
        // 终端ip
        $inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);
        // 随机字符串
        $inputObj->SetNonce_str(self::getNonceStr());
    ...
    public static function orderQuery ($inputObj, $timeOut = 6) {
        ...
        $inputObj->SetAppid(C('WECHAT_INFO.appid'));//公众账号ID
        $inputObj->SetMch_id(C('WECHAT_INFO.mchid'));//商户号
    ...
    {
        //如果不需要上报数据
        if(C('WECHAT_INFO.report_level') == 0){
            return;
        } 
        //如果仅失败上报
        if(C('WECHAT_INFO.report_level') == 1 &&
    ...
    {       
        ...
        //如果有配置代理这里就设置代理
        if(C('WECHAT_INFO.CURL_PROXY_HOST') != "0.0.0.0" 
            && C('WECHAT_INFO.CURL_PROXY_PORT') != 0){
            curl_setopt($ch,CURLOPT_PROXY, C('WECHAT_INFO.CURL_PROXY_HOST'));
            curl_setopt($ch,CURLOPT_PROXYPORT, C('WECHAT_INFO.CURL_PROXY_PORT'));
        }
        ...
        if($useCert == true){
            ...
            curl_setopt($ch,CURLOPT_SSLCERT, C('WECHAT_INFO.sslcert_path'));
            curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
            curl_setopt($ch,CURLOPT_SSLKEY, C('WECHAT_INFO.sslkey_path'));
        }
        //post提交方式
        ...
        if($data){
            ...
            $error = curl_errno($ch);
            curl_close($ch);
            throw new \Exception('curl出错，错误码: ' . json_encode($error));
        }
    }
...
class WxPayDataBase {
    ...
    **/
    ...
    public function MakeSign () {
        ...
        //签名步骤二：在string后加入KEY
        $string = $string . '&key=' . C('WECHAT_INFO.paykey');
        //签名步骤三：MD5加密
        $string = md5($string);
        //签名步骤四：所有字符转为大写
...
class WxPayWithdraw extends WxPayDataBase {
